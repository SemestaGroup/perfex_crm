...
class Invoices extends AdminController
{
    ...
    public function record_invoice_payment_ajax($id)
    {
        ...
        $data['payments'] = $this->payments_model->get_invoice_payments($id);
        $data['banks'] = $this->invoices_model->get_banks();
    ...
    public function get_transfer_details($id)
    {
        ...
        $this->load->model('invoices_model');
        $transfers = $this->invoices_model->get_transfer_from($id);

        $response = [];
        if ($transfers) {
            foreach ($transfers as $t) {
                if(!isset($t['invoiceid'])){
                    $response[] = [
                        'id' => isset($t['id']) ? $t['id'] : null,
                        'transfer_amount' => isset($t['transfer_amount']) ? $t['transfer_amount'] : (isset($t['amount']) ? $t['amount'] : ''),
    ...
    public function connect_payment()
    {
        // Expecting POST with transfer_details (transfer id), invoiceid, date, note and transactionid
        if (!$this->input->post()) {
            show_404();
        }

        $transfer_id = $this->input->post('transfer_details');
        $invoiceid = $this->input->post('invoiceid');
        $date = $this->input->post('date');
        $note = $this->input->post('note');
        $txid = $this->input->post('transactionid');

        if (empty($transfer_id) || empty($invoiceid)) {
            set_alert('danger', 'Missing transfer or invoice information');
            redirect(admin_url('invoices/list_invoices/' . $invoiceid));
        }

        // Load accounting model to get transfer details
        $this->load->model('accounting/accounting_model');
        $transfer = $this->accounting_model->get_transfer($transfer_id);

        if (!$transfer) {
            set_alert('danger', 'Transfer not found');
            redirect(admin_url('invoices/list_invoices/' . $invoiceid));
        }
        ...
        $this->db->update(db_prefix() . 'acc_transfers', ['invoiceid' => $invoiceid]);


        $amount = isset($transfer->transfer_amount) ? $transfer->transfer_amount : 0;

        $payment_data = [
            'amount' => $amount,
            'invoiceid' => $invoiceid,
            'date' => $date ? $date : date('Y-m-d'),
            'paymentmode' => $transfer->transfer_from_bank,
            'note' => $note,
            'paymentmode' => 2,
            'transactionid' => $txid ? $txid : 'TX' . date('YmdHis') . $invoiceid,
        ];

        $this->load->model('payments_model');
        $id = $this->payments_model->add($payment_data);
    ...
    {
        echo 'test';
        require_once(APPPATH . '../modules/accounting/models/Accounting_model.php');
        $accounting_model = new Accounting_model();
        var_dump($accounting_model->get_account_types());
    }

    public function get_transfer_from_bank($id = '')
    {
        require_once(APPPATH . '../modules/accounting/models/Accounting_model.php');
        $accounting_model = new Accounting_model();
        $banks = $accounting_model->get_transfers_from_bank($id);
    }
}
