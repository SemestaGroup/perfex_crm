...
$(document).ready(function(){
  "use strict";
    ...
    // when unit select changes, recalc hidden grams
    $(document).on('change', '.commodity_list-add-edit select[name="unit_id"]', function(){
      var $form = $(this).closest('form');
      var $weight = $form.find('input[name="weight"]');
      if ($weight.length) {
        convertWeightOnChange($weight[0]);
      }
    });
});
...
$('#hot-display-license-info').empty();
  ...
  // Called from the input onchange and when unit select changes
  function convertWeightOnChange(el) {
    try {
      var $el = $(el);
      var $formEl = $el.closest('form');
      if (!$formEl.length) $formEl = $('.commodity_list-add-edit').first();
      var $weightInput = $formEl.find('input[name="weight"]');
      var $unitSelect = $formEl.find('select[name="unit_id"]');
      var $weightInGramsInput = $formEl.find('input[name="weight_in_grams"]');
      if ($weightInput.length && $unitSelect.length) {
        var rawWeight = $weightInput.val();
        var unitText = $unitSelect.find('option:selected').text() || $unitSelect.val();
        var grams = convertWeightToGrams(rawWeight, unitText);
        if ($weightInGramsInput.length) {
          $weightInGramsInput.val(grams);
        } else {
          $formEl.append('<input type="hidden" name="weight_in_grams" value="'+grams+'">');
        }
      }
    } catch (err) {
      // conversion failure should not block interaction
      console.error('Weight conversion failed', err);
    }
  }
...
// Convert various units to grams. Assumes density = 1 for volume units (1 L = 1000 g, 1 ml = 1 g)
function convertWeightToGrams(value, unitName) {
  "use strict";
  if (typeof value === 'undefined' || value === null || value === '') {
    return value;
  }
  // Normalize numeric value (allow comma decimal)
  ...
  var u = '';
  if (typeof unitName !== 'undefined' && unitName !== null) {
    u = unitName.toString().toLowerCase().trim();
  }
  ...
  // fallback: try to detect unit substring
  if (u.indexOf('kg') !== -1) return (num * 1000).toString();
  if (u.indexOf('mg') !== -1) return (num / 1000).toString();
  if (u.indexOf('ml') !== -1) return num.toString();
  if (u.indexOf('l') !== -1) return (num * 1000).toString();
  if (u.indexOf('g') !== -1) return num.toString();

  // unknown unit -> return original numeric string
  return num.toString();
}
