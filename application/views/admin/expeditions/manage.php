            ...
            <div class="col-md-12">
                ...
                <div class="tw-mb-2">
                    <div class="_buttons sm:tw-space-x-1 rtl:sm:tw-space-x-reverse">
                        <!-- <?php if (staff_can('create', 'expeditions')) { ?> -->
                        <a href="<?= admin_url('expeditions/add_expedition'); ?>"
                        ...
                        <a href="#" class="btn btn-default pull-right btn-with-tooltip toggle-small-view hidden-xs"
                            onclick="toggle_small_view('.table-expeditions','#expeditions'); return false;"
                            data-toggle="tooltip"
                    ...
                    <div class="col-md-12" id="small-table">
                        <div class="panel_s">
                            <div class="panel-body">
                                ...
                                <?= form_hidden('id', $expeditionsid); ?>
                                <div class="panel-table-full">
                                    <?php $this->load->view('admin/expeditions/table_html', ['withBulkActions' => true]); ?>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    $(function() {
        initDataTable('.table-expeditions', admin_url + 'expeditions/table', [0], [0], {},
                <?= hooks()->apply_filters('expeditions_table_default_order', json_encode([6, 'desc'])); ?>
            )
            .column(1).visible(false, false).columns.adjust();

        init_expeditions();

        $('#expedition_convert_helper_modal').on('show.bs.modal', function() {
            var emptyNote = $('#tab_expedition').attr('data-empty-note');
            var emptyName = $('#tab_expedition').attr('data-empty-name');
        ...
        $('body').on('click', '#expeditions_confirm_convert', function() {
            var parameters = new Array();
            if ($('input[name="expedition_convert_invoice_type"]:checked').val() == 'save_as_draft_true') {
                parameters['save_as_draft'] = 'true';
            }
            parameters['include_name'] = $('#inc_name').prop('checked');
            parameters['include_note'] = $('#inc_note').prop('checked');
            window.location.href = buildUrl(admin_url + 'expeditions/convert_to_invoice/' + $('body').find(
                '.expedition_convert_btn').attr('data-id'), parameters);
        });
    });
</script>
</body>
