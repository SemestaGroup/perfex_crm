...
class Accounting extends AdminController
{
    ...
    public function transfer_table()
    {
        // if ($this->input->is_ajax_request()) {
            ...
            // Build a map of invoiceid => formatted_number for all invoiceids present in the transfers
            $invoiceIds = [];
            foreach ($rResult as $aRow) {
                if (!empty($aRow['invoiceid'])) {
                    $invoiceIds[] = (int) $aRow['invoiceid'];
                }
            }
            $invoiceMap = [];
            if (!empty($invoiceIds)) {
                $this->db->where_in('id', $invoiceIds);
                $invoicesRows = $this->db->get(db_prefix() . 'invoices')->result_array();
                foreach ($invoicesRows as $inv) {
                    $invoiceMap[$inv['id']] = isset($inv['formatted_number']) ? $inv['formatted_number'] : format_invoice_number($inv['id']);
                }
            }
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = _d($aRow['date']);
                // If transfer has an invoiceid set, display the invoice formatted number, otherwise empty
                $row[] = (isset($invoiceMap[$aRow['invoiceid']]) ? $invoiceMap[$aRow['invoiceid']] : '');
